...
public class InteractMenu {
    ...
    private static void addAttestation() {
        ...
        if (examOrTest.toLowerCase().equals("экзамен")) {
            UL.add(new Exam(subject, day, month, hour, minute, teacherName, teacherSurname));
        }
        else if (examOrTest.toLowerCase().equals("зачет")) {
            UL.add(new Test(subject, day, month, hour, minute, teacherName, teacherSurname));
        }
        else {
    ...
    private static void removeAttestation() {
        ...
        switch (answer) {
            case "1":
                ...
                System.out.print("час проведения : "); hour = checkIntegerInput();
                System.out.print("минута проведения : "); minute = checkIntegerInput();
                UL.remove(new Attestation(subject, day, month, hour, minute, teacherName, teacherSurname) {
                    @Override
                    public int pass() {
                        return 0;
                    }
